        ...
        */
        ...
        for (let x = 0; x < 16; x++) {
            ...
            if (hasTree) {
                ...
                for (let y = 0; y < treeSize; y++) setBlock(x, pY + y + 1, ItemIds.OAK_WOOD);
                [
                    [0, 0], [0, 1], [0, 2],
                    [1, -1], [1, 0], [1, 1], [1, 2],
                    [-1, -1], [-1, 0], [-1, 1], [-1, 2],
                    [2, 0], [2, -1], [-2, 0], [-2, -1]
                ].forEach(pos => setBlock(pos[0] + x, pos[1] + pY + treeSize + 1, ItemIds.OAK_LEAVES));
            }
            for (let y = 1; y < max(round(pY), 55); y++) {
                ...
                if (y < pY) {
                    ...
                    const oreType = types[rand(0, types.length - 1)];
                    const oreId = [ItemIds.COAL_ORE, ItemIds.IRON_ORE, ItemIds.DIAMOND_ORE][oreType];
                    const ore = (x, y) => {
                        if (getBlock(x, y) || x > 15 || x < 0 || getPY(x) - y <= 3 || y <= 0) return false;
                        setBlock(x, y, oreId);
                ...
                } else setBlock(x, y, ItemIds.WATER);
            }
